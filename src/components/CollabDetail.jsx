            ...
            <div className="max-w-4xl mx-auto px-6 -mt-32 relative z-10">
              <div className="bg-white p-12 rounded-[3rem] shadow-2xl border border-slate-50">
                <div className="flex flex-wrap gap-3 mb-6">
                  <span className="px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest text-white" style={{ backgroundColor: colors.peach }}>{collab.type}</span>
                  <span className="px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest bg-slate-100 text-slate-500">
                ...
                <h1 className="text-5xl font-serif font-bold text-slate-900 mb-8">{collab.name}</h1>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12 border-b border-slate-100 pb-12">
                   <div className="space-y-6">
                      <div className="flex items-start gap-4">
                        <div className="p-3 rounded-2xl bg-[#fff2d2] text-[#f8b1d2]"><Calendar size={20}/></div>
                        <div>
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Waktu Pelaksanaan</p>
                          <p className="font-bold text-slate-700">{collab.date}</p>
                        </div>
                      </div>
                      <div className="flex items-start gap-4">
                        <div className="p-3 rounded-2xl bg-[#dadafb] text-[#f8b1d2]"><MapPin size={20}/></div>
                        <div>
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Lokasi Acara</p>
                          <p className="font-bold text-slate-700">{collab.location}</p>
                        </div>
                      </div>
                   </div>
                   <p className="text-slate-500 leading-relaxed text-lg italic">&ldquo;{collab.fullDesc}&rdquo;</p>
                </div>
                ...
                <div className="mt-12 p-8 rounded-[2rem] bg-slate-50 flex flex-col md:flex-row items-center justify-between gap-6">
                  <div>
                    <h4 className="font-bold text-xl mb-1">Tertarik Kolaborasi?</h4>
                    <p className="text-sm text-slate-500">Bawa pengalaman floral ke komunitas atau brand Anda.</p>
                  </div>
                  <button 
                    onClick={() => window.open(getWaLink(`Halo Florisse! Saya tertarik untuk mendiskusikan peluang kolaborasi workshop.`), '_blank')}
                    className="px-8 py-4 rounded-full text-white font-bold text-sm shadow-lg transition hover:scale-105"
